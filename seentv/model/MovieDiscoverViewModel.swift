...
class MovieDiscoverViewModel : ObservableObject {
    ...
    @Published var nowPlayingResults: [Movie] = []
...
//     https://api.themoviedb.org/3/trending/all/day?api_key=91b0dea6fc09258822bd56c6197f8690
    ...
    func loadNowPlaying() {
        Task {
            let url = URL(string: "https://api.themoviedb.org/3/movie/now_playing?api_key=\(MovieDiscoverViewModel.apiKey)")!
            do {
                let (data, _) = try await URLSession.shared.data(from: url)
                let trendingResults = try JSONDecoder().decode(TrendingResults.self, from: data)
                nowPlayingResults = trendingResults.results
            } catch {
                print(error.localizedDescription)
            }
        }
